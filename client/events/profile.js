    ...
    "submit #form-profile": function (e, template) {
        ...
        Meteor.call("updateProfile", profile, function (error) {
            if (error) {
                ...
                swal("Erro ao atualizar pefil", "Entre em contato com administrador do sistema!", "error")
            } else {
                Router.go("profile"); // Redirect user if registration succeeds
                swal("Pefil atualizado com sucesso", "", "success")
            }
        });
    }
});

Template.updatePassword.events({
